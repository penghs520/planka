...
如需修改样式，请编辑 `planka-ui/src/components/common/CompactModal.vue` 文件。
...
**文件结构**：
- `planka-ui/src/utils/compactModal.ts` - 工具函数和类型定义
- `planka-ui/src/components/common/CompactModal.vue` - 全局样式组件
- `planka-ui/src/App.vue` - 导入 CompactModal 组件以加载样式
...
封装 `localStorage` 操作，自动添加 `planka_` 前缀避免命名冲突，支持类型安全的存取。
...
实际存储在 localStorage 中的键名会自动添加 `planka_` 前缀：

```typescript
setItem('userPreference', data)  // 实际存储键: planka_userPreference
```
...
// 存储失败时自动捕获错误并打印日志
try {
  localStorage.setItem('planka_key', JSON.stringify(value))
} catch {
...
`AssertUtils` 提供参数校验的断言方法，校验失败时抛出 `BizException`。用于替代传统的 if-throw 模式，使代码更简洁。
...
```java
import util.cn.planka.common.AssertUtils;
...
User user = userRepository.findById(id)
    .orElseThrow(() -> new BizException(CommonErrorCode.DATA_NOT_FOUND, "用户"));
```
...
1. **异常类型**：所有断言失败都会抛出 `BizException`
...
```java
import util.cn.planka.api.card.ConditionYieldBuilder;
import request.cn.planka.api.card.Yield;
...
import expression.cn.planka.domain.TextExpressionTemplate;
...
import model.cn.planka.infra.cache.card.CardBasicInfo;
import card.cn.planka.domain.CardId;
...
public class CardTypeService {
    ...
    public CardTypeDefinition getCardType(CardTypeId id) {
        return cardTypeCacheQuery.getById(id)
            .orElseThrow(() -> new BizException(CommonErrorCode.DATA_NOT_FOUND, "卡片类型"));
    }
...
```java
import service.cn.planka.api.schema.FieldConfigQueryService;
import inheritance.dto.cn.planka.api.schema.FieldConfigListWithSource;
...
| 工具 | 路径 | 用途 |
|------|------|------|
| StringUtils | `planka-common/util/StringUtils.java` | UUID 生成、字符串转换 |
| SnowflakeIdGenerator | `planka-common/util/SnowflakeIdGenerator.java` | 分布式 ID 生成 |
| BizException | `planka-common/exception/BizException.java` | 业务异常基类 |
| Result | `planka-common/result/Result.java` | 统一 API 响应 |
| PageResult | `planka-common/result/PageResult.java` | 分页结果封装 |
