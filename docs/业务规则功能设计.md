...
- **配置层级**：实体类型级别（每个规则隶属于某个实体类型）
- **触发方式**：事件驱动，系统自动触发（区别于卡片动作的用户手动触发）
- **配置入口**：实体类型配置页的「业务规则」标签页
...
public class RuleExecutionLog {
    ...
    private CardTypeId cardTypeId;                // 实体类型ID（用于分表）
...
执行日志按 `cardTypeId` 进行分表存储，便于按实体类型查询和管理大量日志数据。
...
执行日志存储在按实体类型分表的 `rule_execution_log_{cardTypeId}` 表中。
