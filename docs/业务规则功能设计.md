...
- **配置层级**：__PLANKA_EINST__级别（每个规则隶属于某个__PLANKA_EINST__）
- **触发方式**：事件驱动，系统自动触发（区别于卡片动作的用户手动触发）
- **配置入口**：__PLANKA_EINST__配置页的「业务规则」标签页
...
├── cardTypeId: CardTypeId             # 所属__PLANKA_EINST__
...
| 动作类型 | 说明 | 关键属性 |
|----------|------|----------|
| DISCARD_CARD | 回收卡片 | target, reasonTemplate |
| ARCHIVE_CARD | 存档卡片 | target |
...
│  │  CreateCardActionExecutor   │  CreateLinkedCardActionExecutor │   │
...
└── action/
    ...
    ├── DiscardCardAction.java       # 回收卡片动作
    ├── ArchiveCardAction.java       # 存档卡片动作
...
public class RuleExecutionLog {
    ...
    private CardTypeId cardTypeId;                // __PLANKA_EINST__ID（用于分表）
...
执行日志按 `cardTypeId` 进行分表存储，便于按__PLANKA_EINST__查询和管理大量日志数据。
...
```json
{
  ...
  "orgId": "org-001",
  "name": "需求完成时自动存档子任务",
  "description": "当需求移动到已完成状态时，自动存档所有关联的子任务",
...
```json
{
  ...
  "actions": [
    {
      "actionType": "COMMENT_CARD",
      "target": { "targetType": "CURRENT_CARD" },
      "contentTemplate": "任务 ${card.title} 的优先级已变更，请及时关注。"
    }
  ],
  "enabled": true
}
...
执行日志存储在按__PLANKA_EINST__分表的 `rule_execution_log_{cardTypeId}` 表中。
