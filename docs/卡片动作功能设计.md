...
「卡片动作」是在__PLANKA_EINST__级别配置的可执行操作，用户可在卡片详情页点击触发执行。

### 1.1 功能定位

- **配置层级**：__PLANKA_EINST__级别（单层配置，直接挂载在__PLANKA_EINST__下）
- **触发位置**：卡片详情页（暂不支持卡面，看板视图尚未开发）
- **配置入口**：__PLANKA_EINST__配置页的「卡片动作」标签页

### 1.2 动作分类

| 类别 | 说明 | 示例 |
|------|------|------|
| 生命周期动作 | 固定显示，控制卡片生命周期 | 回收、存档、还原 |
...
| 方法 | 端点 | 说明 |
|------|------|------|
| GET | `/api/v1/schemas/card-actions/by-card-type/{cardTypeId}` | 查询__PLANKA_EINST__的所有动作配置 |
...
| 动作 | 显示条件 | 执行行为 |
|------|----------|----------|
| 回收 | 卡片状态为 ACTIVE | 设置 cardCycle=DISCARDED |
| 存档 | 卡片状态为 ACTIVE | 设置 cardCycle=ARCHIVED |
