...
「卡片动作」是在实体类型级别配置的可执行操作，用户可在卡片详情页点击触发执行。

### 1.1 功能定位

- **配置层级**：实体类型级别（单层配置，直接挂载在实体类型下）
- **触发位置**：卡片详情页（暂不支持卡面，看板视图尚未开发）
- **配置入口**：实体类型配置页的「卡片动作」标签页
...
| 方法 | 端点 | 说明 |
|------|------|------|
| GET | `/api/v1/schemas/card-actions/by-card-type/{cardTypeId}` | 查询实体类型的所有动作配置 |
