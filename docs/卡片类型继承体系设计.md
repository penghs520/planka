# __PLANKA_EINST__继承设计文档

## 1. 功能概述

EntityCardType（__PLANKA_EINST__）支持从多个 AbstractCardType（特征类型）继承，以获得：

1. **自定义属性** (FieldDefinition) - 全局属性定义，继承时直接合并
2. **关联关系** (LinkType) - 当 `sourceCardTypeId` / `targetCardTypeId` 与 __PLANKA_EINST__ 及其 `parentTypeIds` 匹配时，该关联类型对该实体可用
3. **属性配置** (CardTypeFieldConfig) - __PLANKA_EINST__级别的属性配置，支持覆盖

### 1.1 设计原则

- **查询驱动**：__PLANKA_EINST__不维护属性/配置的 ID 列表，通过 `cardTypeId` 反向查询
- **自有优先**：EntityCardType 自有配置优先级最高，自动覆盖继承配置
- **冲突检测**：只有未被自有配置覆盖的 fieldId 才需要检测父类冲突

## 2. 模型设计

### 2.1 AbstractCardType（特征类型）
...
### 2.2 EntityCardType（__PLANKA_EINST__）
...
| 实体 | 关联字段 | 说明 |
|------|----------|------|
| FieldDefinition | cardTypeId | 属性定义所属的__PLANKA_EINST__ |
| CardTypeFieldConfig | cardTypeId | 属性配置所属的__PLANKA_EINST__ |
| LinkTypeDefinition | sourceCardTypeId, targetCardTypeId | 关联类型单侧约束的__PLANKA_EINST__（可空表示不限制） |
...
// 查询 sourceCardTypeId / targetCardTypeId 为空（不限制）或与 allCardTypeIds 相交的 LinkType
```
...
public class CardTypeInheritanceResolver {
    ...
    /**
     * 解析__PLANKA_EINST__的完整配置（包含继承）
     * @param entityCardType __PLANKA_EINST__
     * @return 合并后的完整配置
     */
...
1. **特征类型不支持继承**：只有 EntityCardType 可以继承 AbstractCardType
