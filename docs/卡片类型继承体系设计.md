...
EntityCardType（实体类型）支持从多个 AbstractCardType（特征类型）继承，以获得：

1. **自定义属性** (FieldDefinition) - 全局属性定义，继承时直接合并
2. **关联关系** (LinkType) - 当 `sourceCardTypeId` / `targetCardTypeId` 与 实体类型 及其 `parentTypeIds` 匹配时，该关联类型对该实体可用
3. **属性配置** (CardTypeFieldConfig) - 实体类型级别的属性配置，支持覆盖

### 1.1 设计原则

- **查询驱动**：实体类型不维护属性/配置的 ID 列表，通过 `cardTypeId` 反向查询
...
| 实体 | 关联字段 | 说明 |
|------|----------|------|
| FieldDefinition | cardTypeId | 属性定义所属的实体类型 |
| CardTypeFieldConfig | cardTypeId | 属性配置所属的实体类型 |
| LinkTypeDefinition | sourceCardTypeId, targetCardTypeId | 关联类型单侧约束的实体类型（可空表示不限制） |
