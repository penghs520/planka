...
import cn.agilean.kanban.domain.formula.FormulaId;
 ...
 * belongTo: 所属的卡片类型ID
 * <p>
 * 注意：Jackson 多态注解已集中配置在 {@link cn.agilean.kanban.domain.schema.definition.SchemaDefinition} 接口上。
 */
@Setter
@Getter
...
public abstract class FieldConfig extends AbstractSchemaDefinition<FieldConfigId> {
     ...
     * 是否在标签下方显示提示文本
     */
    @JsonProperty("showHintBelowLabel")
    protected Boolean showHintBelowLabel;

    /**
     * 是否以 tooltip 形式显示提示（在属性名称右侧显示问号图标）
     */
    @JsonProperty("showHintAsTooltip")
    protected Boolean showHintAsTooltip;
