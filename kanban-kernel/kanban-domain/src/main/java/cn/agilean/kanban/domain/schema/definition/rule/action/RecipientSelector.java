package cn.agilean.kanban.domain.schema.definition.rule.action;
...
public class RecipientSelector {
    ...
    /**
     * 字段ID列表（selectorType=FROM_FIELD时使用，支持多选）
     * <p>
     * 可以是多个人员字段，结果取并集。
     */
    ...
    public static class SelectorItem {
         ...
         * 来源标识，用于区分操作人属性和卡片字段
         * <p>
         * 值为 "OPERATOR" 表示操作人的属性，值为 "CARD" 或 null 表示卡片字段
         */
        @JsonProperty("source")
        ...
        public static SelectorItem fromOperatorField(String fieldId) {
            SelectorItem item = new SelectorItem();
            item.setSelectorType(SelectorType.FROM_FIELD);
            item.setFieldId(fieldId);
            item.setSource("OPERATOR");
            return item;
        }
