...
public class SortAndPageConverter {
     ...
     */
    public static SortAndPage toProto(cn.agilean.kanban.api.card.request.SortAndPage sortAndPage) {
        if (sortAndPage == null) {
        ...
        if (sortAndPage.getSorts() != null) {
            for (cn.agilean.kanban.api.card.request.Sort sort : sortAndPage.getSorts()) {
     ...
     * 转换分页信息
     */
    private static Page toProtoPage(cn.agilean.kanban.api.card.request.Page page) {
        if (page == null) {
     ...
     * 转换排序项
     */
    private static Sort toProtoSort(cn.agilean.kanban.api.card.request.Sort sort) {
        if (sort == null) {
     ...
     * 注意：目前简化处理，仅支持内置字段排序
     */
    private static SortField toProtoSortField(cn.agilean.kanban.api.card.request.SortField sortField) {
        if (sortField == null) {
     ...
     * 转换排序方式
     */
    private static SortWay toProtoSortWay(cn.agilean.kanban.api.card.request.SortWay sortWay) {
        if (sortWay == null) {
            return SortWay.Asc;
        }
