package cn.agilean.kanban.card.service.cardDetail;

import cn.agilean.kanban.api.card.dto.CardDTO;
import cn.agilean.kanban.api.card.dto.CardDetailResponse;
import cn.agilean.kanban.api.card.dto.CardDetailResponse.*;
import cn.agilean.kanban.api.card.request.Yield;
import cn.agilean.kanban.api.card.request.YieldField;
import cn.agilean.kanban.api.card.request.YieldLink;
import cn.agilean.kanban.api.schema.service.FieldConfigQueryService;
import cn.agilean.kanban.card.service.core.CardQueryService;
import cn.agilean.kanban.common.result.Result;
import cn.agilean.kanban.common.util.SystemSchemaIds;
...
import cn.agilean.kanban.domain.schema.definition.fieldconfig.FieldType;
...
import cn.agilean.kanban.domain.schema.definition.template.detail.DetailHeaderConfig;
import cn.agilean.kanban.domain.schema.definition.template.detail.FieldItemConfig;
import cn.agilean.kanban.domain.schema.definition.template.detail.SectionConfig;
import cn.agilean.kanban.domain.schema.definition.template.detail.TabConfig;
import cn.agilean.kanban.domain.stream.StatusId;
import cn.agilean.kanban.infra.cache.schema.SchemaCacheService;
import cn.agilean.kanban.infra.cache.schema.query.CardDetailTemplateCacheQuery;
...
public class CardDetailService {
    ...
    public Result<CardDetailResponse> getCardDetail(CardId cardId, String operatorId) {
        try {
            ...
            // 2. 获取字段配置
            List<cn.agilean.kanban.domain.schema.definition.fieldconfig.FieldConfig> domainFieldConfigs =
            ...
            // 6. 转换模板为 DTO
            DetailTemplateDTO template = convertToDTO(templateDef);

            // 7. 转换为字段渲染元数据（与列表视图统一的 FieldRenderConfig）
            List<FieldRenderMetaDTO> fieldRenderMetas =
                    fieldRenderConfigService.toRenderMetas(domainFieldConfigs, cardId, operatorCardId);

            // 8. 计算字段控制配置（必填/只读）
            Map<String, FieldControlDTO> fieldControls =
                    fieldControlService.computeFieldControls(domainFieldConfigs, operatorId);

            // 9. 获取卡片类型信息
            CardTypeInfoDTO cardTypeInfo = getCardTypeInfo(cardTypeId.value());

            // 10. 获取价值流状态信息（如果卡片有 statusId）
            ValueStreamStatusInfoDTO valueStreamStatusInfo = null;
    ...
    private CardDetailTemplateDefinition getTemplateDefinition(
            ...
            List<cn.agilean.kanban.domain.schema.definition.fieldconfig.FieldConfig> fieldConfigs) {
        try {
     ...
     * 将领域模型模板转换为 DTO
     */
    private DetailTemplateDTO convertToDTO(CardDetailTemplateDefinition template) {
        // 转换头部配置
        HeaderConfigDTO header = null;
        if (template.getHeader() != null) {
            DetailHeaderConfig h = template.getHeader();
            header = HeaderConfigDTO.builder()
        ...
        // 转换标签页
        List<TabConfigDTO> tabs = new ArrayList<>();
        if (template.getTabs() != null) {
            for (TabConfig tab : template.getTabs()) {
                List<SectionConfigDTO> sections = new ArrayList<>();
                if (tab.getSections() != null) {
                    for (SectionConfig section : tab.getSections()) {
                        List<FieldItemConfigDTO> fieldItems = new ArrayList<>();
                        if (section.getFieldItems() != null) {
                            for (FieldItemConfig item : section.getFieldItems()) {
                                fieldItems.add(FieldItemConfigDTO.builder()
                        ...
                        sections.add(SectionConfigDTO.builder()
        ...
        return DetailTemplateDTO.builder()
    ...
    private CardDetailTemplateDefinition createDefaultTemplateDefinition(
        ...
        CardDetailTemplateDefinition template = new CardDetailTemplateDefinition(
                cn.agilean.kanban.domain.schema.CardDetailTemplateId.of("default_template"),
                null,
                "默认模板");
        ...
        if (CollectionUtils.isNotEmpty(fieldConfigs)) {
            for (cn.agilean.kanban.domain.schema.definition.fieldconfig.FieldConfig fieldConfig : fieldConfigs) {
     ...
     * 获取卡片类型信息
     */
    private CardTypeInfoDTO getCardTypeInfo(String cardTypeId) {
        try {
            return schemaCacheService.getById(cardTypeId)
                    ...
                    .map(cardType -> CardTypeInfoDTO.builder()
                            .id(cardTypeId)
                            .name(cardType.getName())
                            .build())
                    .orElseGet(() -> {
                        logger.warn("获取卡片类型信息失败，cardTypeId: {}", cardTypeId);
                        return CardTypeInfoDTO.builder().id(cardTypeId).build();
                    });
        } catch (Exception e) {
     ...
     * 获取价值流状态信息
     */
    private ValueStreamStatusInfoDTO getValueStreamStatusInfo(CardTypeId cardTypeId, StatusId statusId) {
        try {
            // 获取价值流定义DTO
                ...
                for (StepConfig step : stream.getStepList()) {
                    for (StatusConfig status : step.getStatusList()) {
                        if (statusId.equals(status.getId())) {
                            ...
                            return ValueStreamStatusInfoDTO.builder()
     ...
     * 根据阶段类型和工作类型获取状态颜色
     */
    private String getStatusColorByStepKind(String stepKind, cn.agilean.kanban.domain.stream.StatusWorkType workType) {
        if (stepKind != null) {
