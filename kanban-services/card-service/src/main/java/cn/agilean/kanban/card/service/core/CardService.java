...
import cn.agilean.kanban.card.event.CardEventPublisher;
...
import cn.agilean.kanban.card.service.permission.exception.PermissionDeniedException;
import cn.agilean.kanban.card.service.permission.model.BatchPermissionCheckResult;
import cn.agilean.kanban.card.service.structure.StructureLinkSyncService;
import cn.agilean.kanban.card.service.validation.FieldValueValidator;
...
import cn.agilean.kanban.domain.card.CardDescription;
import cn.agilean.kanban.domain.field.FieldId;
import cn.agilean.kanban.domain.field.FieldValue;
import cn.agilean.kanban.domain.field.StructureFieldValue;
import cn.agilean.kanban.domain.field.StructureItem;
...
import cn.agilean.kanban.event.card.CardMovedEvent;
...
public class CardService {
    ...
    public Result<Integer> batchUpdateCardStatus(
        ...
        try {
            ...
            cn.agilean.kanban.domain.schema.definition.condition.StatusConditionItem.StatusOperator operator =
                    new cn.agilean.kanban.domain.schema.definition.condition.StatusConditionItem.StatusOperator.Equal(sourceStatusId);
            ...
            for (CardDTO card : cards) {
                if (allowedCardIds.contains(CardId.of(card.getId().value()))) {
                    CardMovedEvent event = eventPublisher.buildMoveEvent(
                            ...
                            card.getStatusId(),
                            cn.agilean.kanban.domain.stream.StatusId.of(targetStatusId),
                            String.valueOf(operatorId.value()));
                    events.add(event);
                }
            }
            ...
            // 5. 批量更新状态
            cardRepository.batchUpdateStatus(allowedCardIds, cn.agilean.kanban.domain.stream.StreamId.of(streamId),
                    cn.agilean.kanban.domain.stream.StatusId.of(targetStatusId), String.valueOf(operatorId.value()));
    ...
    private List<FieldConfig> getFieldConfigsForCardType(CardTypeId typeId) {
        Result<cn.agilean.kanban.api.schema.dto.inheritance.FieldConfigListWithSource> result =
