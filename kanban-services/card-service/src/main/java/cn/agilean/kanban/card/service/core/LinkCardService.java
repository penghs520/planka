...
import cn.agilean.kanban.card.service.structure.StructureSyncResult;
...
import cn.agilean.kanban.domain.field.LinkedCard;
...
import cn.agilean.kanban.event.card.CardLinkUpdatedEvent;
...
public class LinkCardService {
    ...
    private final cn.agilean.kanban.card.service.permission.CardPermissionService permissionService;
    ...
    public LinkCardService(SchemaCacheService schemaCacheService,
                           ...
                           cn.agilean.kanban.card.service.permission.CardPermissionService permissionService) {
    ...
    public Result<Void> updateLink(UpdateLinkRequest request, String orgId, String operatorId,
        ...
        try {
            ...
            // 0. 权限检查（如果未跳过）
            if (!request.isSkipPermissionCheck()) {
                try {
                    permissionService.checkFieldEditPermission(
                            CardId.of(Long.parseLong(cardId)),
                            CardId.of(Long.parseLong(operatorId)),
                            Set.of(),  // 普通属性为空
                            Set.of(linkFieldId),  // 关联属性
                            Map.of(linkFieldId, targetCardIds)  // 对端卡片ID
                    );
