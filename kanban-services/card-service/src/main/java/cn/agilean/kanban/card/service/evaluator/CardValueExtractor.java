...
import cn.agilean.kanban.domain.schema.definition.condition.ReferenceSource;
...
public class CardValueExtractor {
    ...
    public CardDTO getCardByReferenceSource(ReferenceSource source, CardDTO targetCard, CardDTO memberCard) {
        ...
        if (source instanceof ReferenceSource.CurrentCard currentCard) {
            return resolveCardByPath(targetCard, currentCard.getPath());
        }

        if (source instanceof ReferenceSource.Member member) {
            return resolveCardByPath(memberCard, member.getPath());
        }
     ...
     */
    private CardDTO resolveCardByPath(CardDTO card, Path path) {
        if (card == null) {
            return null;
        }
