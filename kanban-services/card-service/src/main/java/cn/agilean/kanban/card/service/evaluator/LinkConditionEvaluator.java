...
import cn.agilean.kanban.domain.schema.definition.condition.LinkConditionItem;
...
public class LinkConditionEvaluator extends AbstractConditionEvaluator {
    ...
    public boolean evaluate(LinkConditionItem item, CardDTO targetCard, CardDTO memberCard) {
        ...
        if (subject.path() == null || subject.path().linkNodes().isEmpty()) {
            return evaluateSingleCard(item, targetCard, targetCard, memberCard);
        }

        // 获取目标卡片集合并使用短路求值评估
        Set<CardDTO> cards = targetCard.getLinkedCards(subject.path());
        return evaluateCards(cards, card -> evaluateSingleCard(item, card, targetCard, memberCard), operator);
    }

    /**
     * 评估单个卡片
     */
    private boolean evaluateSingleCard(LinkConditionItem item, CardDTO card, CardDTO targetCard, CardDTO memberCard) {
        var subject = item.getSubject();
        var operator = item.getOperator();

        // 获取关联卡片集合
        Set<CardDTO> linkedCards = card.getLinkedCards(subject.linkFieldId().value());
