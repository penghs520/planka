package cn.agilean.kanban.card.service.validation;
...
import cn.agilean.kanban.domain.schema.definition.fieldconfig.ValidationRule;
...
public class FieldValueValidator {
     ...
     * 遇到第一个失败就停止
     */
    private String validateField(CardDTO targetCard,
        ...
        for (ValidationRule rule : fieldConfig.getValidationRules()) {
            ...
            try {
                ...
                boolean conditionMet = conditionEvaluator.evaluate(rule.getCondition(), targetCard, null);

                if (!conditionMet) {
                    // 校验失败，解析错误消息并立即返回
                    String errorMessage = resolveErrorMessage(rule, targetCard, operatorId);
    ...
    private String resolveErrorMessage(ValidationRule rule, CardDTO card, String operatorId) {
        ...
        try {
            CardId memberCardId = operatorId != null ? CardId.of(operatorId) : null;
