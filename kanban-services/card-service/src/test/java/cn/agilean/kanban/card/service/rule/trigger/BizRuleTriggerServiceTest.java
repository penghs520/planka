package cn.agilean.kanban.card.service.rule.trigger;
...
import cn.agilean.kanban.card.service.rule.executor.BizRuleExecutionService;
...
class BizRuleTriggerServiceTest {
    ...
    class GeneralTriggerTests {
        ...
        void shouldExecute_whenRulesMatch() {
            ...
            when(cardRepository.findById(eq(CARD_ID), any(), eq("system"))).thenReturn(Optional.of(card));
            when(cardRepository.findById(eq(CardId.of("user-001")), any(), eq("system"))).thenReturn(Optional.of(card));
