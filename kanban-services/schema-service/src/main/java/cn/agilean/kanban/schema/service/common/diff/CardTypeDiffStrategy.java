package cn.agilean.kanban.schema.service.common.diff;

import cn.agilean.kanban.domain.schema.SchemaSubType;
import cn.agilean.kanban.domain.schema.changelog.ChangeDetail;
import cn.agilean.kanban.domain.schema.changelog.FieldChange;
import cn.agilean.kanban.domain.schema.changelog.FieldChangeType;
import cn.agilean.kanban.domain.schema.changelog.SemanticChange;
...
public class CardTypeDiffStrategy implements SchemaDiffStrategy {
    ...
    private List<SemanticChange> compareAttachmentPermissions(JsonNode before, JsonNode after) {
        List<SemanticChange> changes = new ArrayList<>();
        Map<String, JsonNode> beforeMap = toAttachmentOperationMap(before);
        Map<String, JsonNode> afterMap = toAttachmentOperationMap(after);
    ...
    /**
     * 将附件权限数组转换为以操作类型为key的Map
     * <p>
     * 附件权限支持多选操作类型，一条权限规则可能包含多个操作类型
     */
    private Map<String, JsonNode> toAttachmentOperationMap(JsonNode arrayNode) {
        Map<String, JsonNode> map = new LinkedHashMap<>();
        if (arrayNode != null && arrayNode.isArray()) {
            for (JsonNode item : arrayNode) {
                JsonNode operationsNode = item.path("attachmentOperations");
                if (operationsNode != null && operationsNode.isArray()) {
                    for (JsonNode opNode : operationsNode) {
                        String key = opNode.asText(null);
                        if (key != null) {
                            map.put(key, item);
                        }
                    }
