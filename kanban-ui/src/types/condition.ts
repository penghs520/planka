 ...
 * 条件定义类型
 *
 * 与后端 cn.agilean.kanban.domain.schema.definition.condition 包对应
 * 支持任意深度的嵌套条件结构
 */
...
export type DateValue =
  ...
  | { type: 'RECENT'; amount: number; unit: TimeUnit }    // 最近N天/周/月/季度/年
  | { type: 'FUTURE'; amount: number; unit: TimeUnit }    // 未来N天/周/月/季度/年
...
export type EnumOperator =
  ...
  | { type: 'IN'; optionIds: string[] }           // 包含任一（多选）
  | { type: 'NOT_IN'; optionIds: string[] }       // 不包含任一
  | { type: 'IS_EMPTY' }                          // 为空
  | { type: 'IS_NOT_EMPTY' }                      // 不为空
...
export type StatusOperator =
  ...
  | { type: 'IN'; statusIds: string[] }           // 包含任一
  | { type: 'NOT_IN'; statusIds: string[] }       // 不包含任一
...
export type LifecycleOperator =
  | { type: 'IN'; values: LifecycleState[] }         // 包含任一
  | { type: 'NOT_IN'; values: LifecycleState[] }     // 不包含任一
...
export type LinkOperator =
  | { type: 'IN'; value: LinkValue }      // 关联的卡片包含任一
  | { type: 'NOT_IN'; value: LinkValue }  // 关联的卡片不包含任一
  | { type: 'HAS_ANY' }                   // 有任何关联
  | { type: 'IS_EMPTY' }                  // 没有关联
