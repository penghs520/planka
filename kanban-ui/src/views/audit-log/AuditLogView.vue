...
import { SchemaTypeConfig } from '@/types/schema'
...
const schemaTypeOptions = computed(() => {
  ...
  for (const key of Object.keys(SchemaTypeConfig)) {
    options.push({ value: key, label: t(`common.schemaTypeName.${key}`, key) })
  }
  return options
})
...
function getSchemaTypeLabel(schemaType: string): string {
  return t(`admin.schemaTypeName.${schemaType}`, schemaType)
}
