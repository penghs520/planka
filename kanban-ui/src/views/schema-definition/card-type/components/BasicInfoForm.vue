...
// 获取级联字段的方法
const fetchFieldsByLinkFieldId = async (linkFieldId: string): Promise<FieldOption[]> => {
...
<template>
  <div class="basic-info-form-container">
    ...
    <!-- 编辑模式：继承配置单独一行 -->
    <a-form-item
      v-if="mode === 'edit' && formData.schemaSubType === SchemaSubType.ENTITY_CARD_TYPE"
    >
      <template #label>
...
:deep(.collapsed-form-item .arco-form-item-content),
:deep(.collapsed-form-item .arco-form-item-wrapper-col) {
  display: none;
}
