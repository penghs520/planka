...
function getRuleTitle(rule: AttachmentPermission): string {
  // 兼容存量数据：如果没有 attachmentOperations 数组，使用空数组
  const operations = rule.attachmentOperations || []
  const operationLabels = operations.map(op => t(`admin.cardType.permission.attachmentOperations.${op}`))
...
function createDefaultRule(): AttachmentPermission {
  return {
    attachmentOperations: [AttachmentOperation.DOWNLOAD],
    fieldIds: [],
    cardConditions: [],
    operatorConditions: [],
    alertMessage: '',
