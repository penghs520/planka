...
async function loadFieldOptions() {
  ...
  loading.value = true
  try {
    const fields = await fieldOptionsApi.getFields(cardTypeId)
...
// 获取过滤后的枚举选项
function getFilteredEnumOptions(_fieldId: string, fieldOption: FieldOption | undefined) {
  // 过滤掉禁用的选项
  return fieldOption?.enumOptions?.filter(o => o.enabled !== false) || []
}
        ...
        >
          ...
          <template v-if="getFieldOption(assignment.fieldId)">
            ...
            <a-select
              ...
              :options="getFilteredEnumOptions(assignment.fieldId, getFieldOption(assignment.fieldId)).map(o => ({ label: o.label, value: o.value }))"
              style="width: 100%"
            />
            <!-- 关联类型 -->
            <a-select
