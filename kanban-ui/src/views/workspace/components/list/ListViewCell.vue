...
const isNoPermission = computed(() => {
  if (fieldValue.value?.permissionStatus === 'NO_PERMISSION') return true
  if (props.record.linkedCardPermissions?.[props.column.fieldId] === 'NO_PERMISSION') return true
  return false
})

// 是否为受限字段（无权限），不可编辑
const isFieldRestricted = computed(() => {
  return isNoPermission.value
})
...
function handleStartEdit() {
  ...
  if (isFieldRestricted.value) return
...
<template>
  ...
  >
    ...
    />
    <span
      v-else-if="isNoPermission"
      class="cell-value field-no-permission"
    >***</span>
    <span
      v-else
      class="cell-value"
