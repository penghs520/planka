...
where C: RaftTypeConfig
{
    async fn try_get_log_entries<RB: RangeBounds<u64> + Clone + Debug + OptionalSend>(
        ...
        for item_res in it {
            let (id, val) = item_res.map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
            ...
            let entry: EntryOf<C> = serde_json::from_slice(&val).map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e.to_string()))?;
    ...
    async fn read_vote(&mut self) -> Result<Option<VoteOf<C>>, std::io::Error> {
        self.get_meta::<meta::Vote>().map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))
    }
}
...
where C: RaftTypeConfig
{
    ...
    async fn get_log_state(&mut self) -> Result<LogState<C>, std::io::Error> {
        ...
        let last_log_id = match last {
            None => None,
            Some(res) => {
                let (_log_index, entry_bytes) = res.map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
                let ent = serde_json::from_slice::<EntryOf<C>>(&entry_bytes).map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e.to_string()))?;
                Some(ent.log_id())
            }
        };

        let last_purged_log_id = self.get_meta::<meta::LastPurged>().map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
    ...
    async fn save_vote(&mut self, vote: &VoteOf<C>) -> Result<(), std::io::Error> {
        self.put_meta::<meta::Vote>(vote).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
        self.db.flush_wal(true).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
        Ok(())
    }
        ...
        for entry in entries {
            ...
            self.db
                .put_cf(
                    self.cf_logs(),
                    id,
                    serde_json::to_vec(&entry).map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e.to_string()))?,
                )
    ...
    async fn truncate_after(&mut self, last_log_id: Option<LogIdOf<C>>) -> Result<(), std::io::Error> {
        tracing::info!("truncate_after: {:?}", last_log_id);

        // If last_log_id is None, delete all logs
        // If last_log_id is Some(log_id), delete all logs after log_id.index() (exclusive)
        let from_index = match last_log_id {
            None => 0,
            Some(log_id) => log_id.index() + 1,
        };

        let from = id_to_bin(from_index);
        let to = id_to_bin(0xff_ff_ff_ff_ff_ff_ff_ff);
        self.db.delete_range_cf(self.cf_logs(), &from, &to).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
    ...
    async fn purge(&mut self, log_id: LogIdOf<C>) -> Result<(), std::io::Error> {
        ...
        self.put_meta::<meta::LastPurged>(&log_id).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
...
mod meta {
    ...
    where C: RaftTypeConfig
    {
        ...
        fn read_err(e: impl std::error::Error + 'static) -> StorageError<C> {
            StorageError::new(Self::subject(None), ErrorVerb::Read, C::ErrorSource::from_error(&e))
        }

        fn write_err(v: &Self::Value, e: impl std::error::Error + 'static) -> StorageError<C> {
            StorageError::new(Self::subject(Some(v)), ErrorVerb::Write, C::ErrorSource::from_error(&e))
        }
    }
