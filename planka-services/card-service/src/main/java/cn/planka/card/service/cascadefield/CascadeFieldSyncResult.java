 ...
 */
public record CascadeFieldSyncResult(
        boolean synced,
     ...
     * 返回未执行同步的结果
     */
    public static CascadeFieldSyncResult noSync() {
        return new CascadeFieldSyncResult(false, Map.of(), Map.of());
    }
     ...
     * @return 同步结果
     */
    public static CascadeFieldSyncResult of(Map<String, Set<String>> addedLinks,
                                         Map<String, Set<String>> removedLinks) {
        boolean hasChanges = !addedLinks.isEmpty() || !removedLinks.isEmpty();
        return new CascadeFieldSyncResult(hasChanges, addedLinks, removedLinks);
    }
}
