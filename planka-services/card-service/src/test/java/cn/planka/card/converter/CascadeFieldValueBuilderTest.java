...
@DisplayName("CascadeFieldValueBuilder 单元测试")
class CascadeFieldValueBuilderTest {
    ...
    void build_createsCompleteChain_fromLinkedCards() {
        ...
        // 执行
        CascadeFieldValue result = CascadeFieldValueBuilder.build(sourceCard, definition);

        // 验证
        assertThat(result).isNotNull();
        assertThat(result.getFieldId()).isEqualTo("struct_001");
        assertThat(result.getPermissionStatus()).isNull();

        CascadeItem firstItem = result.getValue();
        assertThat(firstItem).isNotNull();
        assertThat(firstItem.getId()).isEqualTo("1001");
        assertThat(firstItem.getName()).isEqualTo("小队Alpha");

        CascadeItem secondItem = firstItem.getNext();
    ...
    void buildAll_createsMultipleValues() {
        CascadeFieldConfig def1 = createCascadeFieldConfig(
                "struct_001",
                List.of(new CascadeRelationLevelBinding(0, LinkFieldId.of("link_001", LinkPosition.SOURCE), true))
        );
        CascadeFieldConfig def2 = createCascadeFieldConfig(
                "struct_002",
                List.of(new CascadeRelationLevelBinding(0, LinkFieldId.of("link_002", LinkPosition.TARGET), true))
        );
        ...
        Map<String, FieldValue<?>> result = CascadeFieldValueBuilder.buildAll(
                sourceCard,
                List.of(def1, def2)
        );
