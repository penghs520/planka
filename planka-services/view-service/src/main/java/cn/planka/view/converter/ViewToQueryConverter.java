package cn.planka.view.converter;

import cn.planka.api.view.request.UserSortField;
import cn.planka.api.view.request.ViewDataRequest;
import cn.planka.common.page.SortDirection;
...
public class ViewToQueryConverter {
    ...
    private List<Sort> convertViewSorts(List<cn.planka.common.page.SortField> viewSorts) {
        return viewSorts.stream()
                .map(this::convertViewSort)
                .collect(Collectors.toList());
    }

    private Sort convertViewSort(cn.planka.common.page.SortField viewSort) {
        Sort sort = new Sort();
