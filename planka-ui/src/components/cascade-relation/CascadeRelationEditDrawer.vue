...
import { cascadeRelationApi, cardTypeApi, fieldOptionsApi } from '@/api'
import CardTypeSelect from '@/components/card-type/CardTypeSelect.vue'
import SaveButton from '@/components/common/SaveButton.vue'
import CancelButton from '@/components/common/CancelButton.vue'
import type {
  CascadeRelationDefinition,
  CascadeRelationLevel,
  CascadeRelationDefinitionRequest,
} from '@/types/cascade-relation'
...
const props = defineProps<{
  visible: boolean
  mode: 'create' | 'edit'
  cascadeRelation: CascadeRelationDefinition | null
}>()
...
// 表单数据
const formData = ref<CascadeRelationDefinitionRequest>({
  name: '',
  description: '',
  levels: [],
})
...
const drawerTitle = computed(() => {
  return props.mode === 'create' ? t('admin.cascadeRelation.createTitle') : t('admin.cascadeRelation.editTitle')
})
...
function initForm() {
  if (props.mode === 'create') {
  ...
  } else if (props.cascadeRelation) {
    formData.value = {
      name: props.cascadeRelation.name,
      description: props.cascadeRelation.description || '',
      levels:
        props.cascadeRelation.levels?.map((l) => ({
          ...l,
    ...
    editingVersion.value = props.cascadeRelation.contentVersion
    selectedLevelIndex.value = 0
  }
}

// 创建新层级
function createNewLevel(index: number): CascadeRelationLevel {
  return {
    index,
    name: '',
    cardTypeId: '',
...
// 检查层级是否填写完整
function isLevelComplete(level: CascadeRelationLevel): boolean {
...
function addLevel() {
  ...
  if (lastLevel && !isLevelComplete(lastLevel)) {
    Message.warning(t('admin.cascadeRelation.completeLevelFirst'))
    return
  }
...
function removeLevel(index: number) {
  if (formData.value.levels.length <= 1) {
    Message.warning(t('admin.cascadeRelation.minLevelRequired'))
    return
  }
...
// 更新层级数据
function updateLevel(field: keyof CascadeRelationLevel, value: unknown) {
...
function validateForm(): boolean {
  if (!formData.value.name.trim()) {
    Message.error(t('admin.cascadeRelation.relationNameRequired'))
    return false
  }
  ...
  for (let i = 0; i < formData.value.levels.length; i++) {
    ...
    if (!level) continue
    if (!level.name.trim()) {
      Message.error(t('admin.cascadeRelation.levelNameRequired', { index: i + 1 }))
      return false
    }
    if (!level.cardTypeId?.trim()) {
      Message.error(t('admin.cascadeRelation.cardTypeRequired', { index: i + 1 }))
      return false
    }
    if (i > 0 && !level.parentLinkFieldId) {
      Message.error(t('admin.cascadeRelation.parentLinkRequired', { index: i + 1 }))
      return false
    }
  }
...
async function handleSubmit() {
  ...
  try {
    ...
    if (props.mode === 'create') {
      await cascadeRelationApi.create(request)
      Message.success(t('common.message.createSuccess'))
    } else if (props.cascadeRelation?.id) {
      await cascadeRelationApi.update(props.cascadeRelation.id, request, editingVersion.value)
    ...
    <div class="drawer-content">
      ...
      <div class="form-section">
        <a-form layout="vertical" :model="formData">
          <a-form-item :label="t('admin.cascadeRelation.relationName')" field="name" required>
            <a-input
              v-model="formData.name"
              :placeholder="t('admin.cascadeRelation.relationNamePlaceholder')"
              :max-length="50"
            />
          </a-form-item>
        </a-form>
      ...
      <div class="form-section levels-section">
        <div class="section-header">
          <span class="section-title">{{ t('admin.cascadeRelation.levelSettings') }}</span>
        </div>
        ...
        <div class="levels-editor">
          ...
          <div class="levels-list">
            ...
            >
              <span class="level-name" :title="level.name">
                {{ level.name || t('admin.cascadeRelation.levelIndex', { index: index + 1 }) }}
              </span>
              <a-button
            ...
            <a-button class="add-level-btn" size="small" type="text" @click="addLevel">
              ...
              {{ t('admin.cascadeRelation.addLevel') }}
            </a-button>
          </div>
          ...
          <a-form
            ...
            layout="vertical"
          >
            <a-form-item :label="t('admin.cascadeRelation.levelName')" required>
              <a-input
                :model-value="selectedLevel.name"
                :placeholder="t('admin.cascadeRelation.levelNamePlaceholder')"
                :max-length="30"
                @update:model-value="updateLevel('name', $event)"
              />
            </a-form-item>

            <a-form-item :label="t('admin.cascadeRelation.cardType')" required>
              <CardTypeSelect
                ...
                :placeholder="t('admin.cascadeRelation.cardTypePlaceholder')"
                @update:model-value="updateLevel('cardTypeId', $event)"
              />
              <template #extra>
                <span class="form-extra">{{ t('admin.cascadeRelation.cardTypeSingleHint') }}</span>
              </template>
            </a-form-item>

            <a-form-item :label="t('admin.cascadeRelation.owner')">
              <a-select
                :model-value="getOwnerLinkValue()"
                :placeholder="t('admin.cascadeRelation.ownerPlaceholder')"
              ...
              />
              <template #extra>
                <span class="form-extra">{{ t('admin.cascadeRelation.ownerHelp') }}</span>
              </template>
            </a-form-item>

            <!-- 非根层级显示与上级的关联关系 -->
            <template v-if="selectedLevelIndex > 0">
              <a-form-item :label="t('admin.cascadeRelation.parentLink')" required>
                <a-select
                  class="parent-link-select"
                  :model-value="getParentLinkValue()"
                  :placeholder="t('admin.cascadeRelation.parentLinkPlaceholder')"
                ...
                />
                <template #extra>
                  <span class="form-extra">{{ t('admin.cascadeRelation.parentLinkHelp') }}</span>
                </template>
              </a-form-item>
            </template>

            <a-form-item :label="t('admin.cascadeRelation.sortField')">
              <div class="sort-row">
                <a-select
                  class="sort-field-select"
                  :model-value="selectedLevel.sortFieldId"
                  :placeholder="t('admin.cascadeRelation.sortFieldPlaceholder')"
                ...
                <a-select
                  ...
                  :placeholder="t('admin.cascadeRelation.sortDirection')"
      ...
      <div class="form-section">
        <a-form layout="vertical" :model="formData">
          <a-form-item :label="t('admin.cascadeRelation.description')" field="description">
            <a-textarea
              v-model="formData.description"
              :placeholder="t('admin.cascadeRelation.descriptionPlaceholder')"
              :max-length="200"
