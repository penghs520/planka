  ...
  <a-card class="condition-item" :class="{ 'is-incomplete': showError }" :bordered="false" hoverable>
    <a-row :gutter="12" >
      ...
      <a-col v-if="hasFieldSelected" :span="localItem.nodeType === 'LINK' ? 14 : 10">
        ...
        <!-- 日期范围输入 (DATE 类型 BETWEEN 操作符) -->
        <DateRangeValueEditor
          v-else-if="needsValue && isDateType() && getOperatorType() === 'BETWEEN'"
          :model-value="localItem.operator as any"
          @update:model-value="handleDateOperatorChange"
        />

        <!-- 日期输入 (DATE, 系统时间字段 - 非 BETWEEN 操作符) -->
        <DateValueEditor
        ...
        <!-- 价值流状态选择 (STATUS) -->
        <StatusValueEditor
          v-else-if="needsValue && localItem.nodeType === 'STATUS'"
          :model-value="localItem.operator as any"
          :stream-id="getStreamId()"
          @update:model-value="handleStatusOperatorChange"
          @update:stream-id="handleStreamIdChange"
        />
...
import type { ConditionItem, LinkOperator, Path, DateOperator, EnumOperator, LifecycleOperator } from '@/types/condition'
...
import { DateValueEditor, DateRangeValueEditor, EnumValueEditor, LifecycleValueEditor } from './value-editors'
import StatusValueEditor from './value-editors/StatusValueEditor.vue'
...
function getFieldId(): string {
  ...
  // 系统字段（CARD_CYCLE, TITLE, CODE）返回 nodeType
...
const availableOperators = computed(() => {
  ...
  const operatorMap: Record<string, { value: string; label: string }[]> = {
    ...
    STATUS: [
      ...
      { value: 'IN', label: getOperatorLabel('IN') },
      { value: 'NOT_IN', label: getOperatorLabel('NOT_IN') },
      { value: 'REACHED', label: getOperatorLabel('REACHED') },
      { value: 'NOT_REACHED', label: getOperatorLabel('NOT_REACHED') },
      { value: 'PASSED', label: getOperatorLabel('PASSED') },
    ],
  }

  // TITLE 操作符：只支持包含、不包含、为空、不为空
  if (nodeType === 'TITLE') {
    return [
      { value: 'CONTAINS', label: getOperatorLabel('CONTAINS') },
      { value: 'NOT_CONTAINS', label: getOperatorLabel('NOT_CONTAINS') },
...
/**
 * 获取价值流ID（用于 STATUS 类型条件）
 */
function getStreamId(): string {
  if (localItem.value.nodeType === 'STATUS') {
...
/**
 * 处理状态操作符变化（来自 StatusValueEditor）
 */
function handleStatusOperatorChange(operator: any) {
  localItem.value.operator = operator
  emitChange()
}

/**
 * 处理价值流ID变化（来自 StatusValueEditor）
 */
function handleStreamIdChange(streamId: string) {
  if (localItem.value.nodeType === 'STATUS') {
    const subject = localItem.value.subject as any
...
function handleFieldSelect(result: FieldSelectionResult) {
  ...
  // 处理其他字段
    ...
    if (result.fieldId) {
      ...
      if ('streamId' in newItem.subject) {
        // STATUS 类型：fieldId 固定为 'STATUS'，streamId 后续从 StatusOption 中获取
        (newItem.subject as any).fieldId = 'STATUS'
      }
    }
...
function handleOperatorChange(newType: string) {
  ...
  if (!operatorNeedsValue(newType)) {
    ...
    if (nodeType === 'TEXT' || nodeType === 'TITLE' || nodeType === 'CODE') {
      ...
      if (newType === 'BETWEEN') {
        // BETWEEN 操作符使用 start 和 end
        if (!op.start) op.start = { type: 'SPECIFIC', value: '' }
        if (!op.end) op.end = { type: 'SPECIFIC', value: '' }
        delete op.value
      } else {
        // 其他操作符使用 value
      ...
      if (newType === 'IN' || newType === 'NOT_IN') {
        if (!op.statusIds) op.statusIds = []
        delete op.statusId
      } else {
        if (!op.statusId) op.statusId = ''
        delete op.statusIds
      }
