...
import { cascadeFieldOptionsApi, type CascadeNodeDTO } from '@/api/cascade-field-options'
...
const treeData = ref<CascadeNodeDTO[]>([])
...
async function loadTreeData() {
  ...
  loading.value = true
  try {
    const data = await cascadeFieldOptionsApi.queryOptions({
      cascadeFieldId: props.fieldId,
    })
    treeData.value = data
  } catch (error) {
...
// 前端过滤树形数据
function filterTree(nodes: CascadeNodeDTO[], keyword: string): CascadeNodeDTO[] {
  if (!keyword) return nodes

  const lowerKeyword = keyword.toLowerCase()
  const filtered: CascadeNodeDTO[] = []
...
// 递归过滤节点
function filterNode(node: CascadeNodeDTO, lowerKeyword: string): CascadeNodeDTO | null {
  const selfMatch = node.name?.toLowerCase().includes(lowerKeyword)

  // 递归过滤子节点
  let filteredChildren: CascadeNodeDTO[] | undefined
...
// 构建从根到指定节点的完整路径
function buildPathToNode(nodeId: string, nodes: CascadeNodeDTO[]): CascadeItem | null {
  for (const node of nodes) {
    if (node.id === nodeId) {
      // 找到目标节点，返回单节点
      return {
...
function convertToTreeSelectData(nodes: CascadeNodeDTO[]): any[] {
...
const selectedKeys = computed(() => {
  if (!props.modelValue?.value) return []
  // 找到链表的最后一个节点（当前选中的节点）
  let current = props.modelValue.value as CascadeItem | null
  while (current?.next) {
...
<template>
  ...
  >
    <template v-if="!autoOpen" #trigger>
      <div class="cascade-field-trigger" :class="{ disabled }">
        <span v-if="displayValue" class="cascade-field-value">{{ displayValue }}</span>
        <span v-else class="cascade-field-placeholder">{{ placeholder || '请选择' }}</span>
        <span class="cascade-field-arrow">
    ...
    <template #label="{ data }">
      <span class="cascade-field-label">{{ displayValue || data?.title || '' }}</span>
    </template>
