 ...
 * 卡片生命周期状态
 */
export type CardCycle = 'ACTIVE' | 'ARCHIVED' | 'DISCARDED'

/**
 * 字段权限状态
 */
export type FieldPermissionStatus = 'FULL_ACCESS' | 'NO_PERMISSION'
...
interface FieldValueBase {
  /** 属性 ID */
  fieldId: string
  /** 权限状态（可选，null 或 FULL_ACCESS 表示完全权限） */
  permissionStatus?: FieldPermissionStatus
}
...
export interface CardDTO {
  ...
  /** 关联属性权限状态 */
  linkedCardPermissions?: Record<string, FieldPermissionStatus>
  /** 创建时间 */
  createdAt?: string
  /** 创建人 ID */
  createdBy?: string
...
export interface ValueStreamStatusInfo {
  ...
  /** 状态名称 */
  statusName: string
  /** 状态所属阶段类别：TODO, IN_PROGRESS, DONE */
  stepKind?: 'TODO' | 'IN_PROGRESS' | 'DONE'
  /** 状态颜色 */
  color?: string
}
