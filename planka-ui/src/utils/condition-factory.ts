...
export function createDefaultConditionItem(nodeType: NodeTypeValue): ConditionItem {
  switch (nodeType) {
    ...
    case NodeType.STATUS:
      return {
        nodeType: 'STATUS',
        subject: { fieldId: 'STATUS', streamId: '' },
 ...
 */
const SYSTEM_FIELD_NODE_TYPES = ['CARD_CYCLE', 'TITLE', 'CODE']
...
export function isConditionItemComplete(item: ConditionItem): boolean {
  ...
  switch (item.nodeType) {
    ...
    case 'STATUS':
      ...
      // EQ/NE/REACHED/NOT_REACHED/PASSED：需要有效的 statusId
      return typeof operator.statusId === 'string' && operator.statusId.length > 0
    case 'CARD_CYCLE':
...
function isDateValueComplete(dateValue: any): boolean {
  ...
  } else if (dateValue.type === 'RECENT' || dateValue.type === 'FUTURE') {
    // 最近/未来：需要有有效的 amount（正数）和 unit
    return typeof dateValue.amount === 'number' && dateValue.amount > 0 && !!dateValue.unit
  }
