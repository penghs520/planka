...
import { isBuiltinField, CARD_CYCLE_LABELS, CARD_CYCLE_COLORS, getStepKindColor } from '@/types/builtin-field'
...
function getBuiltinFieldDisplayValue(
  ...
  switch (fieldId) {
    case '$createdAt':
      return card.createdAt ? formatDateTime(card.createdAt) : emptyPlaceholder
    case '$updatedAt':
      return card.updatedAt ? formatDateTime(card.updatedAt) : emptyPlaceholder
    case '$archivedAt':
      return card.archivedAt ? formatDateTime(card.archivedAt) : emptyPlaceholder
    case '$discardedAt':
      return card.abandonedAt ? formatDateTime(card.abandonedAt) : emptyPlaceholder
    case '$cardCycle':
      return card.cardCycle ? (CARD_CYCLE_LABELS[card.cardCycle] || card.cardCycle) : emptyPlaceholder
    case '$statusId':
      if (!card.statusId) return emptyPlaceholder
      if (statusOptions) {
    ...
    case '$code':
      return card.customCode || String(card.codeInOrg || emptyPlaceholder)
    default:
...
function formatFieldValueByConfig(
  ...
  switch (type) {
    ...
    case 'NUMBER':
      if (typeof value !== 'number') return emptyPlaceholder
    ...
    case 'ENUM':
      if (!Array.isArray(value) || value.length === 0) return emptyPlaceholder
      if (renderConfig?.type === 'ENUM') {
        ...
        if (enumConfig.options) {
          const labels = value
            ...
            .filter(Boolean)
          return labels.length > 0 ? labels.join('、') : emptyPlaceholder
        }
      }
      // 没有渲染配置时返回 ID
 ...
 * @param emptyPlaceholder 空值占位符，默认为 '-'
 */
export function renderFieldValue(
  ...
  if (isBuiltinField(fieldId)) {
    return getBuiltinFieldDisplayValue(card, fieldId, statusOptions, emptyPlaceholder)
  }

  // 2. 处理特殊字段
  if (fieldId === 'title') {
    return card.title?.displayValue || emptyPlaceholder
  }
  ...
  if (isValidLinkFieldId(fieldId)) {
    ...
    if (!linkedCards || linkedCards.length === 0) return emptyPlaceholder
    // 显示关联卡片的标题，多个用顿号分隔
    return linkedCards
      .map(c => c.title?.displayValue || (c.title?.type === 'PURE' ? c.title.value : null) || emptyPlaceholder)
      .join('、')
  }

  // 4. 获取字段值
  const fieldValue = card.fieldValues?.[fieldId]
  if (!fieldValue) return emptyPlaceholder

  // 5. 获取渲染配置
  const renderConfig = getRenderConfig(fieldId, renderSource)

  // 6. 根据渲染配置格式化字段值
  return formatFieldValueByConfig(fieldValue, renderConfig, emptyPlaceholder)
}

/**
 * 判断是否为内置枚举字段（$cardCycle 或 $statusId）
 */
export function isBuiltinEnumField(fieldId: string): boolean {
  return fieldId === '$cardCycle' || fieldId === '$statusId'
}
 ...
 *
 * @param card 卡片数据
 * @param fieldId 字段 ID（$cardCycle 或 $statusId）
...
export function getBuiltinEnumOptions(
  ...
  if (fieldId === '$cardCycle') {
    const cycle = card.cardCycle
    if (!cycle) return []
    return [{
      id: cycle,
      label: CARD_CYCLE_LABELS[cycle] || cycle,
      color: CARD_CYCLE_COLORS[cycle] || '',
      enabled: true,
    }]
  }
  if (fieldId === '$statusId') {
