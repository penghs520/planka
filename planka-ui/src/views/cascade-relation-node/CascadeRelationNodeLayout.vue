...
import { useCascadeRelationNodeViewsMenu } from '@/composables/useCascadeRelationNodeViewsMenu'
...
const { state: viewsMenu, workspaceLinkForView, reloadMenu } = useCascadeRelationNodeViewsMenu()
...
function isCascadeRelationNodeViewSelected(viewId: string) {
...
const cascadeRelationId = computed(() => route.params.cascadeRelationId as string)
const nodeId = computed(() => route.params.nodeId as string)

const node = computed(() => {
  if (!cascadeRelationId.value || !nodeId.value) return undefined
  return cascadeRelationNav.findNode(cascadeRelationId.value, nodeId.value)
})
...
const basePath = computed(
  () => `/cascade-relation/${cascadeRelationId.value}/node/${nodeId.value}`,
)
...
watch(
  ...
  (sid) => {
    if (sid) {
      void cascadeRelationNav.ensureTree(sid)
    }
  },
  { immediate: true },
)
...
<template>
  <div class="cascade-relation-node-layout">
    ...
    <div class="node-body">
      <aside
        class="node-submenu"
        :aria-label="t('sidebar.cascadeRelationNodeSectionNav')"
      >
        <nav class="node-submenu-nav">
          ...
          <div class="node-submenu-group">
            ...
            >
              <button
                type="button"
                class="cascade-relation-node-create-view-btn"
              ...
              <template v-else-if="hasViewsMenuTree">
                <div class="cascade-relation-node-menu-embed">
                  <MenuNode
                    ...
                    :is-selected="isCascadeRelationNodeViewSelected"
              ...
              >
                {{ t('sidebar.cascadeRelationNodeViewsEmpty') }}
              </div>
            </div>
          </div>
        </nav>
    ...
    <ViewEditForm
      ...
      mode="create"
      :cascade-relation-node-context-id="nodeId || undefined"
...
.cascade-relation-node-create-view-btn {
  display: flex;
  align-items: center;
  width: 100%;
  height: 28px;
...
.cascade-relation-node-create-view-btn:hover:not(:disabled) {
  background: var(--color-fill-2);
  color: rgb(var(--primary-6));
}

.cascade-relation-node-create-view-btn:disabled {
  opacity: 0.45;
  cursor: not-allowed;
}

.cascade-relation-node-menu-embed {
  padding: 2px 0 0;
}

.cascade-relation-node-menu-embed :deep(.group-header) {
  color: var(--color-text-2);
  font-size: 12px;
  padding: 5px 6px;
  border-radius: var(--radius-md, 6px);
...
.cascade-relation-node-menu-embed :deep(.group-header:hover) {
  color: var(--color-text-1);
  background: var(--color-fill-2);
}

.cascade-relation-node-menu-embed :deep(.menu-item) {
...
.cascade-relation-node-menu-embed :deep(.menu-item:hover) {
  background: var(--color-fill-2);
}

.cascade-relation-node-menu-embed :deep(.menu-item.active) {
  background: var(--color-primary-light-1);
  color: rgb(var(--primary-6));
  font-weight: 600;
}

.cascade-relation-node-menu-embed :deep(.menu-item.active .item-icon) {
  color: rgb(var(--primary-6));
}

.cascade-relation-node-menu-embed :deep(.item-icon) {
  color: var(--color-text-3);
}
