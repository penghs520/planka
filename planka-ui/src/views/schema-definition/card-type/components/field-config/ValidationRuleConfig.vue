        ...
        >
          ...
          <div class="rule-form">
            ...
            <a-form-item :label="t('admin.cardType.fieldConfig.validationCondition')">
              ...
              <ConditionEditor
                :ref="(el) => { conditionEditorRefs[index] = el as InstanceType<typeof ConditionEditor> | null }"
...
/**
 * ConditionEditor 组件引用（数组，支持多个规则）
 */
const conditionEditorRefs = ref<(InstanceType<typeof ConditionEditor> | null)[]>([])
...
/**
 * 验证所有启用的规则条件是否完整
 * @returns 验证结果，valid 为 true 表示全部完整
 */
function validate(): { valid: boolean; index?: number } {
  for (let i = 0; i < localRules.value.length; i++) {
    const rule = localRules.value[i]
    const ref = conditionEditorRefs.value[i]
    // 只校验启用的规则
    if (rule?.enabled && ref && !ref.validate()) {
