...
import { IconLock, IconShareInternal, IconCheckCircleFill, IconCloseCircleFill, IconDown, IconRight, IconExport, IconQuestionCircle } from '@arco-design/web-vue/es/icon'
...
import { isBuiltinField, getBuiltinField, getStepKindColor, CARD_CYCLE_LABELS, CARD_CYCLE_COLORS } from '@/types/builtin-field'
...
import type { FieldRenderConfig, DateRenderConfig, TextRenderConfig } from '@/types/view-data'
...
const emit = defineEmits<{
  /** 卡片信息加载完成 */
  loaded: [info: { code: string; title: string; cardCycle?: string }]
}>()
...
const cardCycleColor = computed(() => {
  const style = card.value?.cardCycle
  if (!style || style === 'ACTIVE') return ''
  return CARD_CYCLE_COLORS[style] || '#8c8c8c'
})
...
const statusStepKindMap = ref<Map<string, 'TODO' | 'IN_PROGRESS' | 'DONE'>>(new Map()) // statusId -> stepKind
...
async function loadStatusOptions() {
  ...
  try {
    ...
    // 保存 stepKind 映射
    type StepKindType = 'TODO' | 'IN_PROGRESS' | 'DONE'
...
async function fetchCardDetail() {
  ...
  try {
    ...
    cardTabsStore.updateTab(props.cardId, {
      ...
      code: cardCode.value,
      cardCycle: card.value?.cardCycle,
      loading: false,
    })
...
// 获取字段提示信息元数据
function getFieldHintMeta(fieldConfigId: string): FieldRenderMeta | undefined {
  return fieldRenderMetas.value.find(m => m.fieldId === fieldConfigId)
}
...
// 获取字段渲染配置
function getFieldRenderConfig(fieldConfigId: string): FieldRenderConfig | undefined {
  const meta = fieldRenderMetas.value.find(m => m.fieldId === fieldConfigId)
  const renderConfig = meta?.renderConfig
  ...
  if (isBuiltinField(fieldConfigId)) {
    // 日期类型内置字段
    if (['$createdAt', '$updatedAt', '$archivedAt', '$discardedAt'].includes(fieldConfigId)) {
      return {
        type: 'DATE',
        dateFormat: 'DATETIME'
      } as DateRenderConfig
    ...
    // 文本类型内置字段
    if (['$code', '$description'].includes(fieldConfigId)) {
      return {
        type: 'TEXT'
      } as TextRenderConfig
    }
...
// 获取字段值
function getFieldValue(fieldConfigId: string): FieldValue | undefined {
  // 内置字段的值存储在 card 的一级属性中，FieldDisplay 组件会处理
  // 自定义字段从 fieldValues 中读取
...
// 计算字段实际宽度（考虑 gap 影响）
const MAX_COLUMNS = 4 // 最大列数（基于 25% 最小宽度）

function getFieldItemStyle(item: FieldItemConfig, row: FieldItemConfig[]): Record<string, string> {
  const fieldCount = row.length
  if (fieldCount <= 1) {
    return { flex: '1 1 auto' }
  }
  const flexGrow = item.widthPercent / (100 / MAX_COLUMNS)
  return { flex: `${flexGrow} 1 0` }
}
...
function isFieldEditable(fieldId: string): boolean {
  ...
  if (control?.editable === false) return false
  // 无权限字段不可编辑
  const fieldValue = card.value?.fieldValues?.[fieldId]
  if (fieldValue?.permissionStatus === 'NO_PERMISSION') return false
  // 关联属性无权限不可编辑
  if (card.value?.linkedCardPermissions?.[fieldId] === 'NO_PERMISSION') return false
  return true
}
        ...
        <header class="detail-header">
          ...
          <div class="card-meta-row">
            ...
            <span
              ...
              :style="{ backgroundColor: cardCycleColor, color: '#fff' }"
            >
              {{ cardCycleLabel }}
            </span>
            ...
            <template v-if="isBasicInfoTab(activeTab) || activeTab.tabType === 'CUSTOM'">
              <div class="sections-container">
                <div
                  ...
                  class="section-card"
                >
                  <div v-if="section.name" class="section-header" :class="{ 'is-collapsible': section.collapsible !== false }">
                  ...
                  <div v-show="!section.collapsed" class="section-content" :style="{ gap: fieldRowGap }">
                    <div v-for="(row, rowIndex) in getFieldRows(section)" :key="rowIndex" class="field-row">
                      <div
                        ...
                        class="field-item"
                        :style="getFieldItemStyle(item, row)"
                      >
                        <!-- 描述字段特殊渲染 -->
                        ...
                        <template v-else-if="isValidLinkFieldId(item.fieldConfigId)">
                          <div class="field-label">
                            ...
                            <a-tooltip
                              v-if="getFieldHintMeta(item.fieldConfigId)?.showHintAsTooltip && getFieldHintMeta(item.fieldConfigId)?.hintText"
                              :content="getFieldHintMeta(item.fieldConfigId)?.hintText"
                              position="top"
                            >
                              <IconQuestionCircle class="hint-icon" />
                            </a-tooltip>
                          </div>
                          <div
                            ...
                            <template v-else>
                              ...
                              >
                                <FieldDisplay
                                  ...
                                  :render-config="getFieldRenderConfig(item.fieldConfigId)"
                                  :card="card"
                                  :placeholder="getFieldHintMeta(item.fieldConfigId)?.showHintBelowLabel ? getFieldHintMeta(item.fieldConfigId)?.hintText : ''"
                                />
                              </div>
                            </template>
                          </div>
