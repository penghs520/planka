...
ZGraph 是一个使用 Rust 编写的专为知微打造的高性能图数据库，支持单机和主从模式部署，提供ZGraph-driver for java 以 TCP 协议接口的形式供客户端访问。
...
ZGraph 使用 YAML 格式的配置文件进行配置。默认配置文件路径为 `./zgraph.conf`。
...
常用示例：
- `"zgraph=debug,info"`：zgraph库使用DEBUG级别，其他库使用INFO级别
- `"zgraph=debug,zgraph::database=trace,info"`：zgraph库使用DEBUG级别，database模块使用TRACE级别，其他库使用INFO级别
- `"debug"`：所有库都使用DEBUG级别

### 通过zgraph-cli热更新日志级别
= `"log-level <level>"` - 更新日志级别，支持简单格式(trace/debug/info/warn/error)或复杂格式(zgraph=debug,tokio=info)
...
日志文件的命名格式为 `zgraph.YYYY-MM-DD.log`，例如 `zgraph.2025-03-19.log`。
...
allowed_clients:
  - 127.0.0.1
  - 0.0.0.0
db_path: /tmp/zgraph/data
db_snapshot_path: /tmp/zgraph/snapshots
max_snapshot_files_to_keep: 5
db_cache_size_mb: 1024
db_write_buffer_size_mb: 128
db_max_open_files: 64
...
分别启动三个实例
```shell
./zgraph -c /path/to/node1.conf
./zgraph -c /path/to/node2.conf
./zgraph -c /path/to/node3.conf
```
使用zgraph-cli登录连接到node1，然后执行集群初始化和添加从节点操作
```shell
#初始化集群，此时只有node1，node1为leader
zgraph>init-cluster
#添加node2为leaner
zgraph>add-learner 2 127.0.0.1:13895
#添加node3为leaner
zgraph>add-learner 3 127.0.0.1:13896
#查看集群状态，此时预期node1为leader，node2、node3为leaner，且只有node1为voter
zgraph>cluster-status
#改变节点间的成员关系，此时预期node1为leader，node1、node2、node3都为voter
zgraph>change-membership 1,2,3
#测试leader故障转移
...
### docker启动
1. 配置文件放在 /etc/zgraph.conf
配置内容参考上面
2. 启动
```shell
docker run -d -p 3897:3897 -p 13897:13897 -v /etc/zgraph.conf:/etc/zgraph.conf -v /var/zgraph/data:/app/data -v /var/log/zgraph:/app/logs --name zgraph zgraph:latest-ubuntu-amd64
```

### 使用systemctl管理

ZGraph可以配置为系统服务，使用systemd进行管理。步骤如下：

1. 创建系统服务文件

创建文件 `/etc/systemd/system/zgraph.service`，内容如下：

```ini
[Unit]
Description=ZGraph Database Server
...
User=zgraph
Group=zgraph
WorkingDirectory=/opt/zgraph
ExecStart=/opt/zgraph/zgraph /opt/zgraph/zgraph_config.json
Restart=on-failure
RestartSec=5
LimitNOFILE=65536

# 确保日志目录存在并有权限
ExecStartPre=/bin/mkdir -p /tmp/zgraph_logs
ExecStartPre=/bin/chown -R zgraph:zgraph /tmp/zgraph_logs
...
- 启动服务：
```bash
sudo systemctl start zgraph
```

- 停止服务：
```bash
sudo systemctl stop zgraph
```

- 重启服务：
```bash
sudo systemctl restart zgraph
```

- 查看服务状态：
```bash
sudo systemctl status zgraph
```
...
- 使用systemd日志：
```bash
sudo journalctl -u zgraph
```

- 查看最近的日志：
```bash
sudo journalctl -u zgraph -f
```

- 直接查看日志文件：
```bash
ls -l /tmp/zgraph_logs/
cat /tmp/zgraph_logs/zgraph.$(date +%Y-%m-%d).log
```

## 客户端访问

ZGraph 服务器通过 TCP 协议接受客户端连接，使用 Protocol Buffers 进行数据序列化。请参考 zgraph-driver 项目了解如何使用 Java 客户端进行访问。 