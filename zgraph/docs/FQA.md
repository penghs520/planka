...
2. **添加学习者节点**：将新节点以学习者身份加入集群
   ```shell
   zgraph> add-learner 4 127.0.0.1:13897
   zgraph> add-learner 5 127.0.0.1:13898
   ```
3. **提升为正式成员**：将学习者节点提升为具有投票权的正式成员
   ```shell
   zgraph> change-membership 1,2,3,4,5
   ```
...
为了确保集群稳定性，zgraph在实现中增加了Leader保护机制：
...
**安全操作**（包含Leader）：
```shell
zgraph> change-membership 1,2,4  # ✅ 可以执行，包含Leader节点1
```

**危险操作**（不包含Leader）：
```shell
zgraph> change-membership 2,3,4  # ❌ 被拒绝，不包含Leader节点1
```
