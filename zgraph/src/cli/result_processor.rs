...
impl ResultProcessor {
    ...
    fn extract_field_value(card: &Card, field_name: &str) -> Result<String> {
        let value = match field_name {
            ...
            // 时间字段
            "created_at" => Self::format_timestamp(card.created_at),
            "updated_at" => Self::format_timestamp(card.updated_at),
            "archived_at" => {
    ...
    /// 格式化标题
    fn format_title(title: &crate::proto::zgraph::model::Title) -> String {
        if let Some(title_type) = &title.title_type {
            match title_type {
                crate::proto::zgraph::model::title::TitleType::Pure(pure) => {
                    pure.value.clone()
                }
                crate::proto::zgraph::model::title::TitleType::Joint(joint) => {
                    joint.name.clone()
                }
            }
        } else {
