...
fn cleanup_old_log_files(log_path: &str, max_files: u32) {
    ...
    for entry in entries.flatten() {
        ...
        // 只处理.log文件且文件名以zgraph开头
        if path.is_file() {
            if let Some(filename) = path.file_name().and_then(|n| n.to_str()) {
                if filename.starts_with("zgraph") && filename.ends_with(".log") {
                    // 获取文件修改时间
...
pub fn init_logger(config: &ServerConfig) {
    INIT_LOGGER.call_once(|| {
        ...
        // 创建文件输出器，使用日期格式化的文件名：zgraph.2025-03-19.log
        let file_appender = tracing_appender::rolling::Builder::new()
            .rotation(rotation)
            .filename_prefix("zgraph")
            .filename_suffix("log")
            .build(&log_path)
...
mod tests {
    ...
    pub fn test_update_log_level() {
        ...
        // 更新为复杂格式
        match update_log_level("zgraph=debug,tokio=info,info") {
            Ok(msg) => println!("{}", msg),
            Err(e) => println!("更新失败: {}", e),
        }
        
        // 测试复杂格式下的日志过滤
        debug!("这是一条zgraph的DEBUG日志，应该可以看到");
    ...
    pub fn test_logger_filter_debug() {
        ...
        let config = ServerConfig {
            ...
            log_level: Some(String::from("zgraph=debug,tokio=info,info")),
            log_rotation: None,
            // 其他字段以默认值初始化
            ..Default::default()
        };
        ...
        // 测试zgraph的子模块
        debug!(target: "zgraph::database", "这是一条zgraph子模块的DEBUG日志，应该可以看到");
        ...
        println!("测试完成，请查看日志文件：/tmp/zgraph_test_logs/zgraph.*.log");
    }
}