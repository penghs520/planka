...
use crate::raft::app::ZgraphRaftApp;
...
use crate::raft::network::ZgraphRaftNetwork;
...
pub async fn start_raft_node(
    ...
    // 创建网络层，用于节点间通信
    let network = ZgraphRaftNetwork::new();
    ...
    // 创建应用实例
    let app = Data::new(ZgraphRaftApp {
        node_id: node_id,
...
mod tests {
    ...
    impl StoreBuilder<TypeConfig, RocksLogStore<TypeConfig>, StateMachineStore> for Builder {
        ...
        > {
            ...
            let raft_data_path = format!("/tmp/zgraph/raft-data-{}", test_id);
            let snapshots_path = format!("/tmp/zgraph/snapshots-{}", test_id);
            ...
            let sm = StateMachineStore::new(
                ...
                snapshots_path,
                5, // 测试环境默认保留5个快照
            ).await.map_err(|e| StorageError::read_state_machine(BoxedErrorSource::from_error(&e)))?;
            Ok(((), log_store, sm))
        }
    }
