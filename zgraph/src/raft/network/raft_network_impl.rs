...
pub struct ZgraphRaftNetwork {
    pub clients: Arc<HashMap<String, reqwest::Client>>,
}

impl ZgraphRaftNetwork {
    pub fn new() -> Self {
        Self {
...
impl RaftNetworkFactory<TypeConfig> for ZgraphRaftNetwork {
    type Network = ExampleNetworkConnection;

    async fn new_client(&mut self, target: NodeId, node: &Node) -> Self::Network {
        ExampleNetworkConnection {
            owner: ZgraphRaftNetwork::new(),
            target,
