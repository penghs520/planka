...
use crate::proto::zgraph::common::Path;
use crate::proto::zgraph::query::{self, condition_node::NodeType, LogicOperator};
...
fn get_refer_date_values<'a, T: Transaction<'a>>(
    ...
    for vertex in source_vertices {
        let timestamp = match field_id.as_str() {
            "created_at" => Some(vertex.created_at as i64 * 1000),
            "updated_at" => Some(vertex.updated_at as i64 * 1000),
